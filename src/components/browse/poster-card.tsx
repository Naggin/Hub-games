  ...
  return (
    <Link
      ...
      className={cn(
        ...
        variant === "row" && "w-[260px] shrink-0 snap-start sm:w-[300px]",
      )}
    >
      <article
        className={cn(
          "overflow-hidden rounded-xl border border-neon-cyan/15 bg-card/70 transition duration-300",
          "group-hover:border-neon-cyan/50 group-hover:box-glow-cyan",
      ...
      >
        <div className="relative aspect-[460/215] overflow-hidden">
          ...
          <div className="absolute inset-x-0 bottom-0 space-y-1.5 p-3">
            <h3 className="line-clamp-1 text-sm font-medium text-glow-cyan group-hover:text-neon-cyan">
              {title}
            </h3>
            <p className="line-clamp-2 text-[11px] leading-snug text-muted-foreground opacity-0 transition duration-300 group-hover:opacity-100 group-focus-within:opacity-100 motion-reduce:opacity-100">
              {synopsis}
            </p>
            {communityTake && (
